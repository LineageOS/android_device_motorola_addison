...
#define STM_FORCE_DOWNLOAD_MSG  "Use -f option to ignore version check eg: stml0xx boot -f\n"
...
int stm_version_check(int fd, bool check)
{
    ...
    ioctl(fd, STML0XX_IOCTL_GET_VERNAME, ver_string);
    sprintf(ver_file_name, "%s%s.txt", STM_VERSION_FILE, ver_string);
    DEBUG("STML0XX version file name %s\n", ver_file_name);
...
int stm_downloadFirmware( int fd, FILE *filep)
{
    ...
    ret = ioctl(fd, STML0XX_IOCTL_SETSTARTADDR, &address);
...
int main(int argc, char *argv[])
{
    ...
    DEBUG("Start STML0XX  Version-1 service\n");
    ...
    if( fd < 0) {
        LOGERROR("Unable to open stml0xx driver: %s\n",strerror(errno))
        ret = STM_FAILURE;
        goto EXIT;
    }
    ...
    if ((emode == BOOTLOADER) || (emode == BOOTFACTORY)) {
        if (emode == BOOTLOADER) {
            ret = ioctl(fd, STML0XX_IOCTL_GET_VERNAME, ver_string);
            sprintf(fw_file_name, "%s%s.bin", STM_FIRMWARE_FILE, ver_string);
            LOGINFO("STML0XX file name %s\n", fw_file_name)
    ...
    if( emode == TBOOT) {
        DEBUG("Ioctl call to send STM to boot mode\n");
                ret = ioctl(fd,STML0XX_IOCTL_TEST_BOOTMODE, &temp);
    ...
    if( emode == TREAD) {
        ...
        DEBUG( "%02x: ", hexinput[0]);
                ret = ioctl(fd,STML0XX_IOCTL_TEST_WRITE,hexinput);
        ...
        for( i= 0; i< hexinput[0]; i++) {
            ret = ioctl(fd,STML0XX_IOCTL_TEST_READ, &temp);
            DEBUG( "%02x ", ret);
        }
    }
    if( emode == TWRITE) {
    ...
    if( emode == TMWRRD) {
        ...
        DEBUG (" %02x bytes: \n",hexinput[1]);
        ret = ioctl(fd,STML0XX_IOCTL_TEST_WRITE_READ,hexinput);
    }
    if( emode == VERSION) {
    ...
    if( emode == DEBUG ) {
        ...
        delay = hexinput[0];
        DEBUG(" %d\n", delay);
        ret = ioctl(fd,STML0XX_IOCTL_SET_DEBUG,&delay);
        if (delay == 0) {
            system("echo 'file stml0xx_core.c -p' > /sys/kernel/debug/dynamic_debug/control");
            system("echo 'file stml0xx_flash.c -p' > /sys/kernel/debug/dynamic_debug/control");
            system("echo 'file stml0xx_ioctl.c -p' > /sys/kernel/debug/dynamic_debug/control");
            system("echo 'file stml0xx_irq.c -p' > /sys/kernel/debug/dynamic_debug/control");
            system("echo 'file stml0xx_queue.c -p' > /sys/kernel/debug/dynamic_debug/control");
            system("echo 'file stml0xx_reset.c -p' > /sys/kernel/debug/dynamic_debug/control");
    ...
    if( emode == FACTORY ) {
        ...
        ret = ioctl(fd,STML0XX_IOCTL_SET_FACTORY_MODE, &temp);
    }
    if( emode == INVALID ) {
    ...
    if (emode == READWRITE) {
        //                    1B      2B       2B    ...
        // stml0xx readwrite [type] [address] [size] [data]
        //
        // read version example: stml0xx readwrite 00 00 01 00 01
        // write example:        stml0xx readwrite 01 00 0D 00 02 CC DD
        ...
        if (argc < 7) {
            ...
            printf("stml0xx readwrite [type] [address] [size] [data]\n");
            printf("read version example: stml0xx readwrite 00 00 01 00 01\n");
            printf("write example:        stml0xx readwrite 01 00 0D 00 02 CC DD\n");
            goto EXIT;
        }
    ...
    if(emode == LOWPOWER_MODE) {
        ...
        if (setting == 0 || setting == 1) {
            ...
            ret = ioctl(fd, STML0XX_IOCTL_SET_LOWPOWER_MODE, &setting);
        } else {
