...
static int sensorhub_enable(struct sensorhub_device_t* device, struct sensorhub_algo_t* algo)
{
    ...
    case SENSORHUB_ALGO_MOVEMENT:
        if (algo->enable) {
            ...
            if (ioctl(context->control_fd, STML0XX_IOCTL_SET_MOTION_DUR, &data) < 0) {
                ALOGE("STML0XX_IOCTL_SET_MOTION_DUR error (%s)", strerror(errno));
                error = -errno;
            }
            data = algo->parameter[1];
            if (ioctl(context->control_fd, STML0XX_IOCTL_SET_ZRMOTION_DUR, &data) < 0) {
                ALOGE("STML0XX_IOCTL_SET_ZRMOTION_DUR error (%s)", strerror(errno));
                error = -errno;
            }
    ...
    if (!error) {
        if (ioctl(context->control_fd, STML0XX_IOCTL_SET_ALGOS, &data) < 0) {
            ALOGE("STML0XX_IOCTL_SET_ALGOS error (%s)", strerror(errno));
            error = -errno;
        }
    }
    if (!error) {
        ...
        uint32_t active_parts, struct sensorhub_req_t* req_info)
{
    ...
    if (ioctl(context->control_fd, STML0XX_IOCTL_SET_ALGO_REQ, bytes) < 0) {
        ALOGE("STML0XX_IOCTL_SET_ALGO_REQ error (%s)", strerror(errno));
        error = -errno;
    } else {
        context->active_parts[algo] = active_parts;

        // ioctl set algos
        if (ioctl(context->control_fd, STML0XX_IOCTL_SET_ALGOS, &algos) < 0) {
            ALOGE("STML0XX_IOCTL_SET_ALGOS error (%s)", strerror(errno));
            error = -errno;
        } else {
    ...
    struct sensorhub_event_t* output)
{
    ...
    if (algo == SENSORHUB_ALGO_ACCUM_MVMT) {
        evt_reg_size = STML0XX_EVT_SZ_ACCUM_MVMT;
    } else {
        evt_reg_size = STML0XX_EVT_SZ_TRANSITION;
    }
    ...
    if (ioctl(context->control_fd, STML0XX_IOCTL_GET_ALGO_EVT, bytes) < 0) {
        ALOGE("STML0XX_IOCTL_GET_ALGO_EVT error (%s)", strerror(errno));
        error = -errno;
    } else {
