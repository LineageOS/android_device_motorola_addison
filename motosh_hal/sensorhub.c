...
#define DRIVER_CONTROL_PATH "/dev/motosh"
#define DRIVER_DATA_NAME "/dev/motosh_ms"
    ...
    struct sensorhub_event_t* output)
{
    ...
    if (algo == SENSORHUB_ALGO_ACCUM_MVMT) {
        ...
        evt_reg_size = MOTOSH_EVT_SZ_TRANSITION;
    }
...
static int sensorhub_poll(struct sensorhub_device_t* device, struct sensorhub_event_t* event)
{
    ...
    ret = read(context->data_pollfd.fd, &buff, sizeof(struct motosh_moto_sensor_data));
    if (ret == 0)
        return 0;
