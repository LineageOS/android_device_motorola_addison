      ...
      mFlushEnabled(0)
{
    ...
    mPendingEvents[Light].sensor = SENSORS_LIGHT_HANDLE;
    mPendingEvents[Light].type = SENSOR_TYPE_LIGHT;

    mPendingEvents[Prox ].version = sizeof(sensors_event_t);
    mPendingEvents[Prox ].sensor = SENSORS_PROXIMITY_HANDLE;
    ...
    mFlushEvents[Light].meta_data.sensor = SENSORS_LIGHT_HANDLE;
    ...
    mFlushEvents[Prox].meta_data.sensor = SENSORS_PROXIMITY_HANDLE;
...
int LightProxSensor::enable(int32_t handle, int en)
{
    ...
    switch (handle) {
        case SENSORS_LIGHT_HANDLE: what = Light; break;
        case SENSORS_PROXIMITY_HANDLE: what = Prox;  break;
    }
...
int LightProxSensor::flush(int32_t handle)
{
    int id=0;
    switch (handle) {
        case SENSORS_LIGHT_HANDLE: id = BIT_L;  break;
        case SENSORS_PROXIMITY_HANDLE: id = BIT_P;  break;
    }
    mFlushEnabled |= id;
    return 0;
}
