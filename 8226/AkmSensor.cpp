      ...
      mFlushEnabled(0)
{
    ...
    mFlushEvents[MagneticField].version = META_DATA_VERSION;
    mFlushEvents[MagneticField].sensor = 0;
    mFlushEvents[MagneticField].type = SENSOR_TYPE_META_DATA;
    mFlushEvents[MagneticField].reserved0 = 0;
    mFlushEvents[MagneticField].timestamp = 0;
    mFlushEvents[MagneticField].meta_data.what = META_DATA_FLUSH_COMPLETE;
    mFlushEvents[MagneticField].meta_data.sensor = ID_M;

    mFlushEvents[Orientation].version = META_DATA_VERSION;
    mFlushEvents[Orientation].sensor = 0;
    mFlushEvents[Orientation].type = SENSOR_TYPE_META_DATA;
    mFlushEvents[Orientation].reserved0 = 0;
    mFlushEvents[Orientation].timestamp = 0;
    mFlushEvents[Orientation].meta_data.what = META_DATA_FLUSH_COMPLETE;
    mFlushEvents[Orientation].meta_data.sensor = ID_O;

    if (data_fd) {
        strcpy(input_sysfs_path, "/sys/class/compass/akm8963/");
...
int AkmSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    if(count && mFlushEnabled & BIT_M) {
        mFlushEnabled &= ~(BIT_M);
        *data++ = mFlushEvents[MagneticField];
        count--;
        numEventReceived++;
    }

    if(count && mFlushEnabled & BIT_O) {
        mFlushEnabled &= ~(BIT_O);
        *data++ = mFlushEvents[Orientation];
        count--;
        numEventReceived++;
    }
